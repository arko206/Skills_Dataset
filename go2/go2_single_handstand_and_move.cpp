    ...
    float vyaw)
{
  ...
  std::cout
      << "[HANDSTAND_MOVE] Sending Handstand Command."
      << std::endl;

  // Wall-clock timestamp for synchronization with the TF logger.
  const auto skill_start_wall_time =
      WallClock::now();

  // Monotonic timestamp for measuring the API-call duration.
  const auto skill_call_start_steady_time =
      SteadyClock::now();

  WriteCommandEvent(
      "HANDSTAND_MOVE_START",
      skill_start_wall_time,
      vx,
      vy,
      vyaw);
  ...
  const int32_t handstand_ret =
   ...
   const int32_t move_ret =
    ...
    if (move_ret != 0)
    {
        ...
        const int32_t close_ret =
            sport_client_.HandStand(false);

        std::cerr
            << "[CLEANUP] HandStand(false), return="
            << close_ret
            << std::endl;
  ...
  const auto skill_call_return_steady_time =
      SteadyClock::now();

  const auto skill_call_return_wall_time =
      WallClock::now();

    
  WriteCommandEvent(
        "HANDSTAND_MOVE_END",
        skill_call_return_wall_time,
        vx,
        vy,
        vyaw);

  const auto skill_call_duration =
      skill_call_return_steady_time - skill_call_start_steady_time;

  const int64_t skill_call_duration_ns =
      std::chrono::duration_cast<std::chrono::nanoseconds>(
          skill_call_duration)
          .count();

  const double skill_call_duration_us =
      static_cast<double>(skill_call_duration_ns) / 1.0e3;

  const double skill_call_duration_ms =
      static_cast<double>(skill_call_duration_ns) / 1.0e6;

  const int64_t start_steady_time_ns =
      std::chrono::duration_cast<std::chrono::nanoseconds>(
          skill_call_start_steady_time.time_since_epoch())
          .count();

  const int64_t return_steady_time_ns =
      std::chrono::duration_cast<std::chrono::nanoseconds>(
          skill_call_return_steady_time.time_since_epoch())
          .count();

  std::cout
      << "[HANDSTAND_MOVE] start_steady_time_ns = "
      << start_steady_time_ns
      << std::endl;

  std::cout
      << "[HANDSTAND_MOVE] return_steady_time_ns = "
      << return_steady_time_ns
      << std::endl;

  std::cout
      << "[HANDSTAND_MOVE] skill_call_duration_ns = "
      << skill_call_duration_ns
      << " ns"
      << std::endl;

  std::cout
      << "[HANDSTAND_MOVE] skill_call_duration_us = "
      << std::fixed
      << std::setprecision(6)
      << skill_call_duration_us
      << " us"
      << std::endl;

  std::cout
      << "[HANDSTAND_MOVE] skill_call_duration_ms = "
      << std::fixed
      << std::setprecision(6)
  ...
  std::cout
      << "[HANDSTAND_MOVE] HandStand() returned successfully."
      << std::endl;
}
...
int main(int argc, char **argv)
{
    if (argc < 7)
    {
        std::cerr
            << "Usage: " << argv[0]
            << " networkInterface mode trialNumber vx vy vyaw"
            << std::endl;

        std::cerr
            << "Example: " << argv[0]
            << " wlx1cbfced3bbc7 HandStandMove 1 0.4 0.0 0.0"
            << std::endl;
    ...
    if (mode != "HandStandMove")
    {
        std::cerr
            << "[ERROR] go2_single_handstand_and_move supports only "
            << "mode=HandStandMove. Received: "
            << mode
            << std::endl;
    ...
    try
    {
        ...
        vx = std::stof(argv[4]);
        vy = std::stof(argv[5]);
        vyaw = std::stof(argv[6]);
    }
    catch (const std::exception &error)
    {
        std::cerr
            << "[ERROR] Invalid trial number or velocity argument: "
            << error.what()
            << std::endl;
    ...
    Custom custom(command_event_file, mode, trial_number);
    ...
    custom.Trigger_HandStandAndMove(
        vx,
        vy,
        vyaw);
